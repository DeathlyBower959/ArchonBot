module.exports = {
  ...
  async execute(message, args, cmd, client, Discord, prefix, profileData, profileModel, serverData, serverModel) {
    ...
    if (args[0] && prefix !== args[0] && args.length === 1 && isNaN(args[0])) {
      ...
      return message.channel.send(prefixChangedEmbed).then(msg => {
        msg.delete({ timeout: 5000 })
      })
    } else if (prefix === args[0]) {
      return message.channel.send(prefixSameEmbed).then(msg => {
        msg.delete({ timeout: 5000 })
      })
    } else if (args[1]) {
      return message.channel.send(prefixLongEmbed).then(msg => {
        msg.delete({ timeout: 5000 })
      })
    } else if (!args[0]) {
      return message.channel.send(prefixShortEmbed).then(msg => {
        msg.delete({ timeout: 5000 })
      })
